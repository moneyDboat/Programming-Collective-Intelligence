...
def drawgptree(tree, jpeg='output/gptree.jpg'):
    w = getwidth(tree) * 100
    ...
    tree.drawnode(draw, w // 2, 20)
    img.save(jpeg, 'JPEG')


def getwidth(tree):
    if type(tree) == paramnode or type(tree) == constnode:
        return 1
    return sum(getwidth(child) for child in tree.children)


def getdepth(tree):
    if type(tree) == paramnode or type(tree) == constnode:
        return 1
    return max(getwidth(child) for child in tree.children) + 1
